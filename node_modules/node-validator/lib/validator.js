...
function run(validator, value, callback) {
  ...
  function onError(message, childName, childValue, data) {
    errors.push({
      parameter : childName,
      value     : childValue,
      message   : message,
...
function isObject() {
  ...
  function validate(value, onError) {
    ...
    for (var parameter in value) {
      ...
      onError('Unexpected value.', parameter, value[parameter], { err_id: "unexpected_value", parameter: parameter });
    }
    ...
    for (var parameterName in rules) {
      ...
      if ((parameterValue === undefined || parameterValue === null) && rule.required) {
        onError('Required value.', parameterName, parameterValue, { err_id: "missing_parameter", parameter: parameterName } );
        continue;
      }
      ...
      if (rule.validator) {
        rule.validator.validate(parameterValue, function(message, childName, childValue, childData) {
          var name;
          if (childName) {
            if (childName[0] === '[') {
          ...
          onError(message, name, childValue !== undefined ? childValue : parameterValue, childData )
        });
      }
    }
  ...
  function validate(value, onError) {
    ...
    if (options) {
      if (options.regex && !options.regex.test(value)) {
        return onError(options.message || 'Invalid value. Value must match required pattern.', null, null, { err_id: "bad_pattern"});
      }
      if (options.expected && options.expected.indexOf(value) == -1){
        return onError(options.message || 'Invalid value. Value must be in [' + options.expected.join(',') + ']' );
      }
    }
    return null;
  }
...
function isBoolean() {
  ...
  function validate(value, onError) {
    ...
    if (typeof value !== 'boolean' && !( typeof value == 'string' && ['true','false'].indexOf( value ) != -1 ) ) {
...
function isAnyObject() {
  ...
  function validate(value, onError) {
    ...
    for (var parameterName in rules) {
      ...
      function bubbleError(message, childName, childValue, childData) {
        var name;
        if (childName) {
          if (childName[0] === '[') {
            name = parameterName + childName;
  ...
  function validate(value, onError) {
    ...
    if (options) {
      if (options.min && value.length < options.min) {
        return onError('Count must be greater than or equal to ' + options.min + '.', 'length', value.length, { err_id: "array_too_short", min: options.min });
      }
      if (options.max && value.length > options.max) {
        return onError('Count must be less than or equal to ' + options.max + '.', 'length', value.length, { err_id: "array_too_long", max: options.max });
      }
    }
    if (validator) {
      for (var j in value) {
        validator.validate(value[j], function(message, childName, childValue, childData) {
          onError(message, childName ? '[' + j + '].' + childName : '[' + j + ']', childValue !== undefined ? childValue : value[j], childData )
        });
      }
    }
    return null;
