...
```javascript
pusher.get({ path: '/channels/[channel_label]', params: params }, callback);
```
...
```javascript
pusher.get({ path: '/channels/[channel_label]/users' }, callback);
```

The `channel_label` in the path must be a [presence channel](http://pusher.com/docs/presence). The structure of the returned JSON is defined in the [REST API reference](http://pusher.com/docs/rest_api#method-get-users).
