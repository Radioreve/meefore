	...
	var User = require('../models/UserModel'),
				...
				User.findById(hostId,{},function(err,user){
					if(!err){
					  if(user.status==='hosting'){
					  	global.socket.emit('create event error', {msg:"Can't host multiple events"});
					  	return;
					  }
	 ...
	 var cancelEvent = function(data){
			...
			Event.findById(eventId, {}, function(err, myEvent){
				if(err){ console.log('Error finding event : '+err); return;}
				myEvent.state = 'canceled';

				myEvent.save(function(err){
					if(err){ console.log('Error canceling event');   return;  }
					global.io.emit('cancel event success', {eventId: eventId, hostId: hostId} );
				});
		 
				myEvent.askersList.forEach(function(asker){
					User.findById(asker.id, {}, function(err, myUser){
						User.update({ _id: myUser._id}, { $pull: {'eventsAskedList': eventId } }, {}, function(err){
							if(err){ console.log('Error updating user on cancelation '); }
						})
					});
				});
			});
			...
			User.findById(hostId, {}, function(err, myHost){
				...
				myHost.save(function(err){
					if(err){ console.log('Error canceling event (Host)'); }
				});
			});
	 };
	 ...
	 var fetchAskers = function(data){
			...
			Event.findById(data.eventId,{},function(err,myEvent){
				if(err){console.log('error finding event based on hostId');}
				else{
					var askersList = myEvent.askersList;
					global.socket.emit('fetch askers success', askersList);
				}
			});
	 };
